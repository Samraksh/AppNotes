...
namespace Samraksh.eMote.Net.Mac.Ping {
    ...
    public static class Global {
        public static EnhancedEmoteLcd Lcd = new EnhancedEmoteLcd();
    ...
    public class Program {
        ...
        readonly MacConfiguration macConfig = new MacConfiguration();
        ...
        public void Initialize() {
            ...
            try {
                ...
                MACBase.Configure(macConfig, myReceiveCB, myNeighborCB);
        ...
        public void NeighborChange(UInt16 noOfNeigbors) {
        }

        private static void Receive(UInt16 noOfPackets) {
            ...
            var rcvMsg = myCSMA.GetNextPacket();
            if (rcvMsg == null) {
                Debug.Print("null");
                return;
            }

            var rcvMessage = rcvMsg.GetMessage();

            //{
            //    var diagInput = new StringBuilder("0> ");
            //    for (var i = 0; i < rcvMsg.Size; i++) {
            //        diagInput.Append(rcvMessage[i]);
            //        diagInput.Append(' ');
            //    }
            //    Debug.Print(diagInput.ToString());
            //}

            var rcvPayLoadChars = Encoding.UTF8.GetChars(rcvMessage, 0, rcvMsg.Size);

            //Debug.Print("1> " + rcvPayLoadChars.Length);

            //{
            //    var diagInput = new StringBuilder("2> ");
            //    for (var i = 0; i < rcvMsg.Size; i++) {
            //        diagInput.Append(rcvPayLoadChars[i]);
            //        diagInput.Append(' ');
            //    }
            //    Debug.Print(diagInput.ToString());
            //}

            var rcvPayLoadStr = new string(rcvPayLoadChars);

            Debug.Print(rcvPayLoadStr);

            var rcvPayLoadVal = Int32.Parse(rcvPayLoadStr);

            Global.Lcd.Display((rcvPayLoadVal / 10) % 10000);



            //HandleMessage(rcvPayload, rcvMsg.Size, rcvMsg.Src, rcvMsg.Unicast, rcvMsg.RSSI, rcvMsg.LQI);
            //}
            /*try{
        ...
        //        private static int _messageCnt;

        //        static void HandleMessage(byte[] msg, UInt16 size, UInt16 src, bool unicast, byte rssi, byte lqi) {
        //            Global.Lcd.Display(_messageCnt++);
        //            Debug.Print("MSG: " + msg[0] + " " + msg[1] + " " + msg[2] + " " + msg[3] + " " + msg[4] + " " + msg[5]);
        //#if TRUE
        //            try
        //            {
        //                /*if (unicast)
