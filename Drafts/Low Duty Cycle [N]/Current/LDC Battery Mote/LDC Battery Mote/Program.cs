...
namespace Samraksh.AppNote.LowDutyCycle {
    ...
    public static class Program {
        ...
        public static void Main() {
            ...
            //  Timer is started in DutyCycleAction
            //  When it ticks, turn the radio and the timer off
            _radioPowerTimer = new Timer(_ => TurnRadioAndTimerOff(), null, Timeout.Infinite, RadioPowerTimerTimeoutMilliSec);
        ...
        private static void PacketReceived(CSMA csma) {
            ...
            if (!packet.Unicast) { return; }

            // Check if this is for us
            var msgBytes = packet.GetMessage();
            var protocolName = System.Text.Encoding.UTF8.GetChars(msgBytes, _commonItems.AppPos, _commonItems.AppLen).ToString();
            // If wrong app name, not for us
            if (protocolName != Common.AppName) { return; }
