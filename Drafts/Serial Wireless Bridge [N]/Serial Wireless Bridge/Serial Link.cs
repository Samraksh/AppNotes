				...
				while (_circularBuffer.IsData)
				{
					...
					switch (currState)
					{
						...
						case ProcessState.Start:
							if (currByte == Messages.MessageType.Election[0])
							{
								messageLen = 0;
								currState = ProcessState.Election;
							}
							else if (currByte == Messages.MessageType.Outgoing[0])
							{
								messageLen = 0;
								currState = ProcessState.Outgoing;
							}
							else if (currByte == Messages.MessageType.Return[0])
							{
								messageLen = 0;
