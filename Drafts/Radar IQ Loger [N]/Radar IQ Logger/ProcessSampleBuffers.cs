	...
	public partial class Program
	{
		...
		private static void WriteSampleBuffers(IQPair iq)
		{
			...
			for (var i = 0; i < ADCBufferSize; i++) {
				SampleSb.Append(iBuff[i]);
				SampleSb.Append(' ');
				SampleSb.Append(qBuff[i]);
				Debug.Print(SampleSb.ToString());
				SampleSb.Clear();
			}
			
		}
		private static readonly StringBuilder SampleSb = new StringBuilder();
	}
}
