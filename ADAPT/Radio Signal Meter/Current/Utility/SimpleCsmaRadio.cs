        ...
        public SimpleCsmaRadio(RadioName radioName, byte ccaSensetime, TxPowerValue txPowerValue, RadioReceivedData radioReceivedData) {
            ...
            try {
                ...
                if (retVal != DeviceStatus.Success) {
                    Debug.Print("Attempt to configure CSMA radio was not successful. Return code: "+retVal);
                ...
                Debug.Print("CSMA configuration error\n" + e);
                Thread.Sleep(Timeout.Infinite);
                throw;
            }
            catch (Exception ex) {
                Debug.Print("Unknown error while configuring CSMA\n" + ex);
                Thread.Sleep(Timeout.Infinite);
                throw;
            }
