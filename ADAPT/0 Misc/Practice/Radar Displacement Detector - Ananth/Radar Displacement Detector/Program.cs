/*--------------------------------------------------------------------
 * Radar Displacement Detector: app note for Adapt
...
namespace Samraksh.AppNote.Adapt.RadarDisplacementDetector {
    ...
    public static class RadarDisplacementDetector {
        ...
        private static readonly ushort[] buffer = new ushort[(int)Detector.BufferSize];
        ...
        public static void Main() {
            ...
            //Create thread that calls AdcBuffer_Callback()
            System.Threading.Thread workerThread;
            workerThread = new Thread(new ThreadStart(AdcBuffer_Callback));

            ushort result;
            int ADC_channel_0 = 0;
            result = analogInput.Read(ADC_channel_0);
            Debug.Print("Sample Result " + result);
            //buffer[0] = result;
        ...
        private static void AdcBuffer_Callback() {
            ...
            for (var i = 0; i < (int)Detector.BufferSize; i++) {
                ...
                SampleData.CurrSample.I = buffer[i];
                ProcessSample();
            }
        ...
        private static void ProcessSample() {
            ...
            // If first 2 values in the buffer is not filled up, return
