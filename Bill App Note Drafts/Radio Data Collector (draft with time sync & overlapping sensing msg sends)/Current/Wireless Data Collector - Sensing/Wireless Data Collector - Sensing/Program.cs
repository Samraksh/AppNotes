...
namespace Samraksh.AppNote {
    ...
    public partial class Program {
        ...
        // A list of timers. Entries are populated wrt a particular sent message to check for timeouts.
        ...
        private static void RadioReceive(CSMA csma) {
            ...
            for (var packetNum = 0; packetNum < numPackets; packetNum++) {
                ...
                // Lock on message timers to avoid race condition when updating
                lock (MessageTimers) {
                    for (var i = 0; i < MessageTimers.Count; i++) {
                        ...
                        // Mark that the message is found
                        messageFound = true;

                        // Calculate stats on reply time
                        var currTicks = DateTime.Now.Ticks;
                        var replyTicks = currTicks - theTimer.StartTime;
                        ...
                        Debug.Print("Time to receive reply: [" + (long)_replyMsMin + "," + (long)replyMs + "," +
                                    (long)_replyMsMax + "]; timeout value used: " + MessageTimeout + "; curr time: " + (long)currTicks + "; timer start time: " + theTimer.StartTime);
                ...
                // Check if any timers outstanding and turn off radio
                var waitingForTimeout = false;
                lock (_csmaRadio) {
                    lock (MessageTimers) {
                        foreach (var theMessageTimer in MessageTimers) {
                            if (!((MessageTimer)theMessageTimer).SeqTimer.IsStopped) {
                                waitingForTimeout = true;
                                break;
                            }
                        }
                        if (waitingForTimeout) return;
                        lock (_csmaRadio) {
                            Debug.Print("\nTurn Radio OFF\n");
        ...
        /// <summary>
        /// Send a message and start radio timer
        /// </summary>
        /// <param name="dest">Destination</param>
        /// <param name="payloadType">Type of payload</param>
        /// <param name="sensedTimeDataPairs">Sensed data (time, data pairs)</param>
        private static void RadioSend(Addresses dest, PayloadTypes payloadType, byte[] sensedTimeDataPairs) {
            ...
            // Lock MessageTimers to avoid race condition with other mods
            lock (MessageTimers) {
                for (byte i = 0; i < (byte)MessageTimers.Count; i++) {
                    if (!(((MessageTimer)MessageTimers[i]).SeqTimer).IsStopped)
                        continue; // If not stopped, keep looking
        ...
        private static void MessageTimeoutCallback(object obj) {
            ...
            // Stop all the timers
            // Use a lock to avoid race conditions with other mods
            lock (MessageTimers) {
                foreach (var theMessageTimer in MessageTimers) {
