...
namespace Samraksh.AppNote.WirelessDataCollector
{
	...
	{
		private static EnhancedEmoteLcd Lcd;

		private static MACBase _mac;
		...
		public static void Main()
		{
			...
			_mac = RadioConfig.GetMAC();
			_mac.OnReceive += RadioReceive;
			RadioUtilities.PrintMacInfo(_mac);
		...
		private static void RadioReceive(IMAC mac, DateTime dateTime, Packet packet)
		{
			// Get the message
			var msgBytes = packet.Payload;

			// Print message details
			//  This is optional
			Debug.Print("\nGot a message from " + RadioUtilities.PacketInfo(packet));

			// Check the message type 
			if (msgBytes[Common.PayloadTypePos] != (byte)Common.PayloadTypes.Reply)
			{
				Debug.Print("Invalid payload type");
				return;
			}
			...
			for (var i = 0; i < Common.ApplicationIdSize; i++)
			{
				...
				Debug.Print("Invalid app identifier.");
				return;
			}

			// All is ok ... set the source ID of the base station
			_baseStationId = packet.Src;
			...
			Debug.Print("Time to receive reply: [" + (long)_replyMsMin + "," + (long)replyMs + "," +
						...
						"; curr time: " +
						currTicks + "; timer start time: " + TimeoutTimer.StartTime);
#endif
