...
namespace Samraksh.AppNote.Utility {
	...
	public class EnhancedEmoteLcd : EmoteLCD {
		...
		private readonly LCD[] _numDigits = new LCD[4];
		...
		/// </summary>
		/// <param name="num">The number to be displayed</param>
		/// <remarks>Avoiding conversion from int to string so as not to impact the heap.</remarks>
		public void Write(int num) {
			for (var i = 3; i >= 0; i--) {
				_numDigits[i] = _digits[num % 10].ToLcd();
				num /= 10;
			}
			Write(_numDigits[0], _numDigits[1], _numDigits[2], _numDigits[3]);
		}
