...
namespace Samraksh.AppNote.DotNow.DataCollector {
    ...
    public static class DataCollector {
        ...
        private static int _dataReferencesWritten, _dataItemsWritten;
        ...
        private static void ProcessSampleBuffer() {
            while (true) {
                ...
                for (var i = 0; i < Ibuffer.Length; i++) {
                    ...
                    if (_sampleCounter % 10 == 1) {
                        Debug.Print(_sampleCounter + " I: " + Ibuffer[i] + " Q: " + Qbuffer[i]);
                    }
                }

                var freeBefore = Dstore.FreeBytes;
                if (freeBefore < IQbuffer.Length * 2) {
                    Debug.Print("Filled up data store");
                    StartSavingCollectionData.Set();
                    return;
                }

                var dref = new DataReference(Dstore, IQbuffer.Length, REFERENCE_DATA_TYPE.UINT16);
                try {
                    ...
                    Debug.Print("Exception writing data reference");
                    throw;
                }

                Debug.Print("");

                var freeAfter = Dstore.FreeBytes;
                Debug.Print("Datastore bytes free: Before " + freeBefore + ", After " + freeAfter + ", Diff " + (freeBefore - freeAfter));

                // Counters for later verification
                _dataReferencesWritten++;
                _dataItemsWritten += IQbuffer.Length;

                var processingTime = DateTime.Now - startProcessing;
                Debug.Print("Processing Time: " + (processingTime.Seconds * 1000 + processingTime.Milliseconds) + ", Callback Interval: " + AdcParameters.CallbackIntervalMs);
                Debug.Print("");
        ...
        private static void WriteToSd() {
            ...
            while (true) {
                ...
                foreach (var theReference in dataReferences) {
                    ...
                    dataItemsRead++;
                    if (dataItemsRead % 10 == 0) {
                        Debug.Print("  Writing sample " + dataItemsRead);
                    }
                    if (theReference.getDataReferenceSize != IQbuffer.Length) {
                        throw new Exception("Data reference size " + theReference.getDataReferenceSize +
                                            " is not equal to IQbuffer length " + IQbuffer.Length);
                    }
        ...
        finished:
            ...
            Debug.Print("Check data store written vs. read");
            var dr = "    Data References " + _dataReferencesWritten + " written, " + dataReferencesRead + " read";
            Debug.Print(_dataReferencesWritten == dataReferencesRead ? "" : "***** " + dr);
            var ditems = "    Data Items " + _dataItemsWritten + " written, " + dataItemsRead + " read";
            Debug.Print(_dataItemsWritten == dataItemsRead ? "" : "***** " + ditems);
        }
