close all;
if ~exist('ArduinoCapture','var')
    [~,~,ArduinoCapture] = xlsread('Arduino Capture.xlsx');
end
...
for Idx = 1 : length(ArduinoCapture)-1
    if ~strcmp(ArduinoCapture(Idx, LineTypeC), '#4') || isnan(ArduinoCapture{Idx+1,1})
        continue;
    end
    
    SampleNo = ArduinoCapture{Idx, SampleNoC};
    IA = ArduinoCapture{Idx, IValC};
    QA = ArduinoCapture{Idx, QValC};
    ...
    SumIA = ArduinoCapture{Idx, SumIC};
    SumQA = ArduinoCapture{Idx, SumQC};
    PrevSampIA = ArduinoCapture{Idx-1, SampIC};
    PrevSampQA = ArduinoCapture{Idx-1, SampQC};
    AdjIA = ArduinoCapture{Idx, AdjIC};
    AdjQA = ArduinoCapture{Idx, AdjQC};
    PrevAdjIA = ArduinoCapture{Idx, PrevAdjIC};
    PrevAdjQA = ArduinoCapture{Idx, PrevAdjQC};
    
    IsCutA = ArduinoCapture{Idx, IsCutC};
    CurrCutsA = ArduinoCapture{Idx, CurrCutsC};
    RunCutsA = ArduinoCapture{Idx, RunCutsC};
    DispA = ArduinoCapture{Idx, DispC};
    ConfA = ArduinoCapture{Idx, ConfC};
    
    PrevAdjIM = ArduinoCapture{Idx-1, AdjIC};
    PrevAdjQM = ArduinoCapture{Idx-1, AdjQC};
    ...
    if PrevSampIA >= 0
        QValM = floor((ArduinoCapture{Idx-1,QValC} + ArduinoCapture{Idx+1,QValC})/2);
    ...
    if PrevSampQA >= 0
        IValM = floor((ArduinoCapture{Idx-1,IValC} + ArduinoCapture{Idx+1,IValC})/2);
