...
//  for everything like I do, fork over $20 to:
//
//  MindSpace Research
//  168 South 425 West
...
#define OS_WRAP_VERSION "OSW_MINI_110706"
...
void printP(const unsigned char* _str, bool _new_line = true);
...
//  For a DOS system the system tic is at 18.2 Hz.
//#define MS_PER_TIC (54.945054945)
//  For this Windows XP box a tic is at 65.5 Hz.
...
class osw_counting_sem
{
  public:
    ...
    // Semtake will return the count if it got the semaphore.
    //  Returns 0 is there is no semaphore.
    int getCount(void);
    int semTake(int _to_wait_ms = OSW_WAIT_FOREVER);
    int semGive(void);
    void semFlush(void);
...
//  Message queues and shared memory normally have the user specify the size,
//  then the objects will malloc() the memory needed for the queue.  For
//  Arduino, RAM is at such a premium this method of doing things just
//  doesn't work in practice.  So created a template where the user can
//  specify size (at compile time).
//
//  I wanted the body of the template in the .cpp file, but I couldn't
//  make that work with the Arduino compiler (Either I'm missing something
//  or it's just one step too much for this compiler.)  So all the code is
//  included here as inline functions.  The compiler seems happy with that.

//  But having all the functions inline makes the declarations unreadable.
//  So, for reference, here are the shared memory class and the message queue
//  class interfaces:

//template <int size>
//class osw_shared_mem {
//  public:
//    osw_shared_mem(void):flag_(0);
//    int write(void* _data, int _size);
//    //  This read() should work inside of interrupts as well.  Assumming
//    //  that interrupts stack up, instead of interrupts interrupting
//    //  other interrupts.  I.e. an interrupt that reads while another
//    //  interrupt is halfway done writing.
...
//
//
//    int pkdQsend(char *_in_msg,  int _size);
//
//    void pkdQflush(void);
//};
...
/////////////////////////////////////////////////
//  Shared Memory  sizeof(osw_shard_mem) == 4
/////////////////////////////////////////////////
...
//  latches so that when other task are interrupted, they can detect it
//  and try the read or write again.
//
...
//////////////////////////////////////////////////////////////////////////////
    void int_write(void* _data, int _size) {
      ...
      //  and if you have more than 16,000 threads trying to use the same
...
/////////////////////////////////////////////////
//  Message Queue   sizeof(osw_message_q) == 21
/////////////////////////////////////////////////
...
//  Outputs: none
//////////////////////////////////////////////////////////////////////////////
    osw_packed_q():ROLLOVER_VALUE(0xff)
    {
      tail_ = 0;
      head_ = 0;
    }
...
//////////////////////////////////////////////////////////////////////////////
                    ...
                    osw_tick_t* _tick = OSW_NULL)
    {
      ...
      if (head_ == tail_) return OSW_SINGLE_THREAD;
      
      // get the message.
      char tmp = msg_array_[head_];
      if (tmp == ROLLOVER_VALUE) {
        //Serial.print("pkdQreceive-Rollover");
        head_ = 0;
        tmp = msg_array_[head_];
      }
      char retsize = tmp;               // get the size.
      ...
      ++head_;
      memcpy(_buf, &msg_array_[head_], retsize);
      head_ += retsize;
      return retsize;
    }
...
//////////////////////////////////////////////////////////////////////////////
                 ...
                 int _size)
    {
      // put the message.
      if (head_ > tail_) {
        if ((_size + 1) <= (head_ - tail_)) {
          msg_array_[tail_] = _size;
          ++tail_;
          memcpy(&msg_array_[tail_], _in_msg, _size);
          tail_ += _size;
        } else {
        ...
        if ((_size + 1) <= (size - tail_)) {
          msg_array_[tail_] = _size;
          ++tail_;
          memcpy(&msg_array_[tail_], _in_msg, _size);
          tail_ += _size;
        } else if ((_size + 1) <= head_) {
          // rollover.
          msg_array_[tail_] = ROLLOVER_VALUE;
          //Serial.print("pkdQsend-Rollover");
          tail_ = 0;
          msg_array_[tail_] = _size;
          ++tail_;
          memcpy(&msg_array_[tail_], _in_msg, _size);
...
//////////////////////////////////////////////////////////////////////////////
  ...
  private:
    char msg_array_[size];
    char head_;
    char tail_;
    const char ROLLOVER_VALUE;
};
