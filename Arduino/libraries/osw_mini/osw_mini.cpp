...
{
  const unsigned char* _str = err2pMsg(_error);
  static char c[40];
  int i = 0;
...
//////////////////////////////////////////////////////////////////////////////
//  function:  osw_interrupt_disable
...
static osw_event_pair osw_evt_event[OSW_MAX_NUM_EVENTS];
static int osw_evt_pairCount;
static osw_packed_q<OSW_EVENT_Q_SIZE> osw_event_q;
...
void* osw_event_service_loop(void* _data)
{
  while (1) {
    int event;
    int result = osw_event_q.pkdQreceive((char*)&event, sizeof(event));
...
{
  if (osw_evt_pairCount == OSW_MAX_NUM_EVENTS) {
...
{
  if (osw_event_q.pkdQsend((char*)&_event, sizeof(_event)) != OSW_OK) {
...
//     _evt_trace (not used in osw_mini), traces the task in the trace log.
...
void idle_task(void)
{
  ...
  osw_tick_t delta_t;
  ++idle_count;
  //  Come up with something for the idle task to do, like toggle a light
  //  or print a dot, no more that 5/sec no less than 1/sec..
